...
for ( i in c("male", "used_transport", "selftested_covid_positive", "smoking", "smoked_previously", "anosmia_past", "lives_alone", "dogs", "cats", "chronical_diseases", "work_status", "study_status", "anosmia_now", "income_level", "work_status", "study_status", "education_level", "lives_alone", "spouse_or_partner", "Atherosclerosis") ) {
    df[[i]] <- as.factor(df[[i]])
}
...
##статистика по партнеру (значимость только в группе молодых)
```{r}
...
#Регрессионный анализ для витамин D, индекса атерогенности, холестерина и ТГ...
```{r}
...
df_upd <- df %>% 
 dplyr::select(male, age, ever_smoker, cigarettes_per_day, Cholesterin, Triglycerides, Atherogenic_index, VitaminD)
...
# Регрессия без факторов

fit_VitaminD <- glm(VitaminD ~ 1, data = df)
summary(fit_VitaminD)

fit_Cholesterin <- glm(Cholesterin ~ 1, data = df)
summary(fit_Cholesterin)


fit_Atherogenic_index <- glm(Atherogenic_index ~ 1, data = df)
summary(fit_Atherogenic_index)


fit_Triglycerides <- glm(Triglycerides ~ 1, data = df)
summary(fit_Triglycerides)


#
ggplot(df, aes(x = cigarettes_per_day, y = Atherogenic_index)) +
  geom_point() + 
  geom_smooth(method = lm, formula = 'y ~ x', se = FALSE)


#возраст и наличие партнера - взаимодействие факторов, курение, пола, уровня образования и трудового статуса

fit_1 <- glm(Cholesterin  ~ age*spouse_or_partner+male+ever_smoker+education_level+work_status+num_people_home, data = df)
summary(fit_1)

ggcoef_model(fit_1, exponentiate = FALSE, 
             intercept = TRUE,
             variable_labels = c(
               age = "Age",
               ...
               work_status = "Worker",
               num_people_home = "Family"
             )
              )

fit_2 <- glm(Atherogenic_index  ~ age*spouse_or_partner+male+ever_smoker+education_level+work_status+num_people_home, data = df)
summary(fit_2)

ggcoef_model(fit_2, exponentiate = FALSE, 
             intercept = TRUE,
             variable_labels = c(
               age = "Age",
               spouse_or_partner = "Live with spouse or partner",
...
fit_3 <- glm(VitaminD   ~ age*spouse_or_partner+male+ever_smoker+education_level+work_status, data = df)
summary(fit_3)

ggcoef_model(fit_3, exponentiate = FALSE, 
             intercept = TRUE,
             variable_labels = c(
               age = "Age",
               spouse_or_partner = "Live with spouse or partner",
...
fit_4 <- glm(Triglycerides   ~ age*spouse_or_partner+male+ever_smoker+education_level+work_status, data = df)
summary(fit_4)


ggcoef_model(fit_4, exponentiate = FALSE, 
             intercept = TRUE,
             variable_labels = c(
               age = "Age",
               spouse_or_partner = "Live with spouse or partner",
...
tab_model(fit_1, fit_2, fit_3, fit_4)


```


#Propensity Score Matching in R - нравится мне, ничего не могу поделать с собой...=(
```{r}
...
match.it <- matchit(lives_alone ~ age + male + smoking, data = mydata, method="nearest", ratio=1)
a <- summary(match.it)


kable(a$nn, digits = 2, align = 'c', 
      caption = 'Table 2: Sizes')


kable(a$sum.matched, digits = 2, align = 'c', 
      caption = 'Table 3: Summary for matched data')
...
wilcox.test(df.match$Atherogenic_index  ~ df.match$lives_alone, paired = FALSE, conf.int = TRUE)

wilcox.test(df.match$Cholesterin  ~ df.match$lives_alone, paired = FALSE, conf.int = TRUE)

wilcox.test(df.match$VitaminD  ~ df.match$lives_alone, paired = FALSE, conf.int = TRUE)

df.match %>% 
  filter(!(is.na(lives_alone))) %>% 
  ggplot(aes(y = VitaminD, x = lives_alone, fill = lives_alone))+
  geom_boxplot()+
  stat_compare_means(label.y = 8)+
  theme_classic()
