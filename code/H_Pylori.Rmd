...
merged_data$smoking_more_30 <- ifelse(merged_data$smoking_experience >= 30, 1, 0)

```


```{r}
cor.test(merged_data$age, merged_data$smoking_experience)

chisq.test(merged_data$age, merged_data$smoking_more_30)
```
...
ggplot(df_for_model, aes(HP_10_prob, fill = as.factor(HP_10_resp_correct)))+
  geom_dotplot()
...
ggplot(df_for_model, aes(HP_10_prob, fill = as.factor(HP_10_resp_correct_2)))+
  geom_dotplot()
...
sensitivity(data = df_for_model, as.factor(IgG_hpylori_10EIU), as.factor(HP_30_resp_1)) # чувствительность
specificity(data = df_for_model, as.factor(IgG_hpylori_10EIU), as.factor(HP_30_resp_1)) # специфичность
```
...
tab_model(fit10_1, fit10_2, fit30_1, fit30_2, show.obs = T, show.r2 = F,
          auto.label = FALSE,
...
```{r}
fit10_int_1 <- glm(IgG_hpylori_10EIU ~ age *spouse_or_partner, data = df_for_model, family = "binomial")
summary(fit10_int_1)
...
```{r}
fit10_int_2 <- glm(IgG_hpylori_10EIU ~ age*under_18 , data = df_for_model, family = "binomial")
summary(fit10_int_2)
...
```{r}
fit10_int_3 <- glm(IgG_hpylori_10EIU ~ age*under_18*spouse_or_partner , data = df_for_model, family = "binomial")
summary(fit10_int_3)
