...
#ML CHAID
```{r}
model_chaid <- caret::train(x = train_data_chaid[, c(1:5, 7:18)], y = train_data_chaid$street_used_mask,
                          method = "chaid",
                          metric = "ROC",
                          trControl = trainControl(method = "boot", number = 100,
                                                   classProbs = T, summaryFunction = twoClassSummary))

pred_chaid <- predict(model_chaid, train_data_chaid)
cm_rpart <- caret::confusionMatrix(pred_chaid, train_data_chaid$street_used_mask, positive = "Yes")
cm_rpart
