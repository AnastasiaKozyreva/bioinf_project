...
ML_data$street_used_mask <- droplevels(ML_data$street_used_mask)
...
#For CHAID
ML_data$age_group <- cut(ML_data$age, breaks = c(17, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 95), labels = c("18-25", "26-30", "31-35", "36-40", "41-45", "46-50", "51-55", "56-60", "61-65", "66-70", "71+"), ordered = T)
ML_data <- ML_data[, -3]
...
```{r}
set.seed(666)
CHAID_data <- logit_subset_3
CHAID_data$age_group <- cut(CHAID_data$age, breaks = c(17, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 95), labels = c("18-25", "26-30", "31-35", "36-40", "41-45", "46-50", "51-55", "56-60", "61-65", "66-70", "71+"), ordered = T)
CHAID_data <- CHAID_data[, -3]
CHAID_data %>%
  mutate(across(.cols = everything(), .fns = as.factor)) -> CHAID_data
CHAID_data$street_used_mask <- droplevels(CHAID_data$street_used_mask)

levels(CHAID_data$male) <- c("No", "Yes")
levels(CHAID_data$travelled) <- c("No", "Yes")
levels(CHAID_data$used_transport) <- c("No", "Yes")
levels(CHAID_data$selftested_covid) <- c("Not tested", "Neg", "Pos")
levels(CHAID_data$street_used_mask) <- c("No", "Yes")
levels(CHAID_data$nosymptoms_past) <- c("No", "Yes")
levels(CHAID_data$no_checkups) <- c("No", "Yes")
levels(CHAID_data$under_18) <- c("No", "Yes")
levels(CHAID_data$over_65) <-  c("No", "Yes")
levels(CHAID_data$spouse_or_partner) <-  c("No", "Yes")
levels(CHAID_data$old_partner) <-  c("No", "Yes", "Maybe")
levels(CHAID_data$times_sick) <- c("Never", "Once", "Twice", "Thrice")
levels(CHAID_data$self_medical_checkups) <- c("No", "Yes")
levels(CHAID_data$other_people) <- c("No", "Yes")



splitted_data <- initial_split(CHAID_data, prop = 0.8, strata = "street_used_mask")
train_data_chaid <- splitted_data %>% training()
test_data_chaid <-  splitted_data %>% testing()
...
#CHAID
```{}
chaid_res_3 <- chaid(street_used_mask ~ ., data = CHAID_data, control = chaid_control(minbucket = 150, minprob = 0.2))

plot(chaid_res_3, gp = gpar(fontsize = 8))
...
#ML CHAID
```{r}
model_chaid_3 <- caret::train(x = CHAID_data[, c(1:5, 7:18)], y = CHAID_data$street_used_mask,
                          method = "chaid",
                          metric = "ROC",
                          trControl = trainControl(method = "cv", number = 50,
                                                   classProbs = T, summaryFunction = twoClassSummary))

pred_chaid <- predict(model_chaid_3, CHAID_data)
cm_rpart <- caret::confusionMatrix(pred_chaid, CHAID_data$street_used_mask, positive = "Yes")
cm_rpart

plot(model_chaid_3$finalModel, gp = gpar(fontsize = 8))
```
